...
########################
# Collect all the .htaccess files from /opt/vcnngr/$name and write the result in the 'htaccess' directory
# Globals:
#   APACHE_*
# Arguments:
#   $1 - App name
...
apache_replace_htaccess_files() {
    ...
    # Default options
    local document_root="${VCNNGR_ROOT_DIR}/${app}"
    local overwrite="yes"
...
ensure_apache_app_configuration_exists() {
    ...
    export document_root="${VCNNGR_ROOT_DIR}/${app}"
...
EOF
    ...
    local -r template_dir="${VCNNGR_ROOT_DIR}/scripts/apache/bitnami-templates"
