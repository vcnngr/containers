...
web_server_execute() {
    ...
    (
        . "/opt/vcnngr/scripts/lib${web_server}.sh"
        . "/opt/vcnngr/scripts/${web_server}-env.sh"
        "$@"
    )
}
...
web_server_list() {
    ...
    for web_server in "${supported_web_servers[@]}"; do
        [[ -f "/opt/vcnngr/scripts/${web_server}-env.sh" ]] && existing_web_servers+=("$web_server")
    done
...
web_server_validate() {
    ...
    if [[ -z "$(web_server_type)" || ! " ${supported_web_servers[*]} " == *" $(web_server_type) "* ]]; then
        ...
        print_validation_error "Could not load the $(web_server_type) web server library from /opt/vcnngr/scripts. Check that it exists and is readable."
    fi
...
web_server_start() {
    ...
    if [[ "${VCNNGR_SERVICE_MANAGER:-}" = "systemd" ]]; then
        systemctl start "vcnngr.$(web_server_type).service"
    else
...
web_server_stop() {
    ...
    if [[ "${VCNNGR_SERVICE_MANAGER:-}" = "systemd" ]]; then
        systemctl stop "vcnngr.$(web_server_type).service"
    else
