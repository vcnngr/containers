...
| `DB_BASE_DIR`                   | Base path for MariaDB files.                                 | `${VCNNGR_ROOT_DIR}/mariadb`   |
| `DB_VOLUME_DIR`                 | MariaDB directory for persisted files.                       | `${VCNNGR_VOLUME_DIR}/mariadb` |
...
The image looks for user-defined configurations in `/opt/vcnngr/mariadb/conf/my_custom.cnf`. Create a file named `my_custom.cnf` and mount it at `/opt/vcnngr/mariadb/conf/my_custom.cnf`.
...
docker run --name mariadb \
    -p 3306:3306 \
    -e ALLOW_EMPTY_PASSWORD=yes \
    -v /path/to/my_custom.cnf:/opt/vcnngr/mariadb/conf/my_custom.cnf:ro \
    -v /path/to/mariadb-persistence:/bitnami/mariadb \
    bitnami/mariadb:latest
```
...
services:
  ...
  ...
    volumes:
      - /path/to/my_custom.cnf:/opt/vcnngr/mariadb/conf/my_custom.cnf:ro
      - /path/to/mariadb-persistence:/bitnami/mariadb
  ...
```
...
```console
docker run --name mariadb  -e ALLOW_EMPTY_PASSWORD=yes -v /path/to/my.cnf:/opt/vcnngr/mariadb/conf/my.cnf:ro bitnami/mariadb:latest
```
...
RUN ini-file set --section "mysqld" --key "collation-server" --value "utf8_general_ci" "/opt/vcnngr/mariadb/conf/my.cnf"
...
To increase the verbosity on intialization or add extra debug information, you can assign the `VCNNGR_DEBUG` environment variable to `true`.
...
- The MariaDB configuration files are not persisted in a volume anymore. Now, they can be found at `/opt/vcnngr/mariadb/conf`.
...
- The user is able to specify a custom file in the default location '/opt/vcnngr/mariadb/conf/my.cnf'.
