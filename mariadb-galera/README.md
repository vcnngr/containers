...
| `MARIADB_GALERA_CONF_DIR`                     | MariaDB Galera configuration directory                                                                                    | `/opt/vcnngr/mariadb/conf`               |
...
| `DB_BASE_DIR`                             | Base path for MariaDB Galera files.                                                                                 | `${VCNNGR_ROOT_DIR}/mariadb`     |
| `DB_VOLUME_DIR`                           | MariaDB Galera directory for persisted files.                                                                       | `${VCNNGR_VOLUME_DIR}/mariadb`   |
...
docker run --name mariadb \
  -e ALLOW_EMPTY_PASSWORD=yes \
  -v /path/to/my.cnf:/opt/vcnngr/mariadb/conf/my.cnf:ro \
  bitnami/mariadb-galera:latest
```
...
RUN ini-file set --section "mysqld" --key "collation-server" --value "utf8_general_ci" "/opt/vcnngr/mariadb-galera/conf/my.cnf"
...
services:
  mariadb:
    ...
    ports:
      - 3306:3307
    volumes:
      - /path/to/my_custom.cnf:/opt/vcnngr/mariadb-galera/conf/my_custom.cnf:ro
